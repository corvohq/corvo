...
fn printHelp() void {
    std.debug.print(
        \\corvo-v2 — single-threaded pipeline server
        \\
        \\Usage: corvo-v2 [options]
        \\
        \\Options:
...
pub fn main() !void {
    ...
    if (config_path_len > 0) {
        ...
        const file = std.fs.cwd().openFile(path, .{}) catch {
            std.debug.print("corvo-v2: failed to open config file: {s}\n", .{path});
            return;
        };
        defer file.close();
        const n = file.readAll(&file_buf) catch {
            std.debug.print("corvo-v2: failed to read config file: {s}\n", .{path});
            return;
        };
        config.loadFile(file_buf[0..n]) catch |err| {
            std.debug.print("corvo-v2: config file error: {s}\n", .{@errorName(err)});
            return;
        };
    }
    ...
    config.validate() catch |err| {
        std.debug.print("corvo-v2: config error: {s}\n", .{@errorName(err)});
        return;
    };
    ...
    if (cluster_mode) {
        ...
        std.debug.print("corvo-v2: cluster node={s}, peers={d}, transport=:{d}, config_hash={x}\n", .{
