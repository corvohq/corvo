...
func (s *Store) HoldJob(id string) error {
	...
	if result.Affected == 0 {
		return fmt.Errorf("job %q not found or cannot be held from current state", id)
	}
	return nil
}
...
func (s *Store) ApproveJob(id string) error {
	...
	if result.Affected == 0 {
		return fmt.Errorf("job %q not found or cannot be approved from current state", id)
	}
	return nil
}
...
func (s *Store) RejectJob(id string) error {
	...
	if result.Affected == 0 {
		return fmt.Errorf("job %q not found or cannot be rejected from current state", id)
	}
	return nil
}
